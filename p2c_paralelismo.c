...
int MPI_BinomialColectiva(void *buff, int count, MPI_Datatype datatype,
	...
	int paso, dest, aux, resultado, proceso, rank, numProcs;
	MPI_Comm_size(comm, &numProcs);
	MPI_Comm_rank(comm, &rank);

	if (count < 0) return MPI_ERR_COUNT;
	if (buff == NULL) return MPI_ERR_BUFFER;
	if (root < 0 || root > (numProcs-1)) return MPI_ERR_ROOT;

	for (paso = 0; /*paso < log2(numProcs)*/; paso++) {
		aux = pow(2, paso);

		if (rank < aux) {	// envio de datos a los siguientes procesos
			dest = rank + aux;
			if(dest >= numProcs) break;	// envio completado

			/* controlar error con var resultado */
			resultado = MPI_Send(buff, count, datatype, dest, 0, comm);
			if(resultado != MPI_SUCCESS) return resultado;

		} else {	// recive de otro proceso y empieza a enviar
			proceso = rank - pow(2, paso);
			if(proceso < pow(2,paso)) {

				/* controlar error con var resultado */
				resultado = MPI_Recv(buff, count, datatype, proceso, 0, comm, MPI_STATUS_IGNORE);
				if(resultado != MPI_SUCCESS) return resultado;
			}
		}
	}
